...
import respiratory_detection as detection 
import respiratory_thermal_face as thermal_face
...
class ThermalFrame(object):
    ...
    def __init__(self, thermal_frame, timestamp, width, height):
        self.timestamp = timestamp
        self.thermal_frame = thermal_frame
        self.grey_frame = utils.rescale(thermal_frame)
        self.thermal_faces = []
        self._detect(width, height)

    def _detect(self, w, h):
        """ Detect all face entities in this frame.
        """
        bounding_boxes, landmarks = detection.get_face_detection(self.grey_frame, w, h)
    ...
    def annotated_frame(self, annotate_breath_rate=True):
